...
页面流程：选择分析 → 录入数据（CSV 上传 / 粘贴 / 内置示例）→ 查看结果。工具主体采用 `ydchen-portfolio` 的米白 / 浅灰 / 赤陶色（`#c15f3c`）视觉系统；`YDchen Tools` 页眉保持原有结构与样式不变。
...
runHeavyTask 统一维护 aria-busy 与三个提交按钮，成功、异常、通信失败和取消路径均需释放。旧 Worker 的迟到消息不能解除新任务的忙碌状态；统计核心不承担 DOM 状态。
